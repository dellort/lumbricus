...
class SubtexTest : Task {
    private {
        LandscapeBitmap[][] mGrid;
        ScrollBar mSubDivBar;
        Button mStart;
        Label mDisplay;
        Time mBegin;
        Time mFrameTime;
    ...
    class Draw : Widget {
        ...
        override void onDraw(Canvas c) {
            if (!mGrid.length)
                return;
            Vector2i tile = mGrid[0][0].size + Vector2i(1);
            foreach (int y, row; mGrid) {
                foreach (int x, LandscapeBitmap b; row) {
                    c.draw(b.image, tile^Vector2i(x,y));
                }
            }
        }
    }
    ...
    this(TaskManager tm, char[] args = "") {
        ...
        mSubDivBar = new ScrollBar(true);
        mSubDivBar.maxValue = 20;
        mSubDivBar.minValue = 1;
        //mSubDiv.onValueChange = &onSubDivChange;
        mSubDivBar.setLayout(WidgetLayout.Expand(true));
        box.add(mSubDivBar);

        mStart = new Button();
        mStart.text = "Start";
        mStart.onClick = &onStart;
        mStart.setLayout(WidgetLayout.Expand(true));
        box.add(mStart);

        mDisplay = new Label;
        mDisplay.setLayout(WidgetLayout.Expand(true));
        box.add(mDisplay);

        Draw d = new Draw();
        box.add(d);

        gWindowManager.createWindow(this, box, "subtex test", Vector2i(2100, 800));
    }

    private void onStart(Button button) {
        Surface img = gGuiResources.get!(Surface)("checkbox_on");
        mGrid = null;
        mSubDiv = mSubDivBar.curValue;
        Vector2i tile = cSize / mSubDiv;
        mGrid = new LandscapeBitmap[][](mSubDiv, mSubDiv);
        foreach (row; mGrid) {
            foreach (ref LandscapeBitmap b; row) {
                b = new LandscapeBitmap(tile);
                b.image.enableCaching = false;
                b.addPolygon([Vector2i(0),Vector2i(tile.x,0),tile,Vector2i(0,tile.y)],
                    Vector2i(0), img, Lexel.SolidSoft);
            }
        }
        mFrames = cFrames;
        mRandom = new Random;
        mRandom.seed(1);
        mFrameTime = Time.init;
        mBegin = timeCurrentTime();
    }
    ...
    override void onFrame() {
        ...
        void blast(Vector2i at) {
            Vector2i tile = mGrid[0][0].size + Vector2i(1);
            foreach (int y, row; mGrid) {
                foreach (int x, LandscapeBitmap b; row) {
                    auto rc = Rect2i.Span(tile^Vector2i(x,y), b.size);
                    auto vr = Vector2i(cRadius + cBlastBorder);
                    if (rc.intersects(Rect2i(-vr, vr) + at)) {
                        b.blastHole(at-rc.p1, cRadius, cBlastBorder, null);
                    }
                }
            }
        }

        auto a = timeCurrentTime();

        for (int n = 0; n < cPerFrame; n++) {
            //not very realistic; game should be a bit more local with
            //  explosions, although not much more in heavy moments
            blast(Vector2i(mRandom.next(cSize.x), mRandom.next(cSize.y)));
        }

        auto b = timeCurrentTime();
        mFrameTime += b - a;

        mFrames--;

        if (mFrames == 0) {
            auto diff = timeCurrentTime() - mBegin;
            mDisplay.text = myformat("subdiv: {}\nframe time: {}\nfull time: {}",
                mSubDiv, mFrameTime, diff);
        }
    }

    static this() {
        TaskFactory.register!(typeof(this))("subtex_test");
    }
}
