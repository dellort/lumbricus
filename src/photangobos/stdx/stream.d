...
import tf = tango.io.device.File;
...
class File: Stream {
  ...
  void open(char[] filename, FileMode mode = FileMode.In) {
    ...
    try {
        tf.File.Style st;
        if ((mode & (FileMode.In | FileMode.Out)) == (FileMode.In | FileMode.Out)) {
            st.access = tf.File.Access.ReadWrite;
        } else if (mode & FileMode.In) {
            st.access = tf.File.Access.Read;
        } else if (mode & FileMode.Out) {
            st.access = tf.File.Access.Write;
        }
        if (mode & FileMode.Out) {
            //xxx: correct?
            st.open = tf.File.Open.Sedate;
        }
        if ((mode & FileMode.OutNew) == FileMode.OutNew) {
            st.open = tf.File.Open.Create;
        }
        st.share = tf.File.Share.ReadWrite;
        //xxx: I don't know
        st.cache = tf.File.Cache.Random;
        hFile = new tf.File(filename, st);
    } catch (IOException e) {
  ...
  //doesn't work anymore, don't try it
  //~this() { close(); }

  override size_t readBlock(void* buffer, size_t size) {
    assertReadable();
    //xxx: Tango throws an error on error... ok
    //     Phobos signals EOF and returns on error... what????????
    uint res;
    try {
        res = hFile.read(buffer[0..size]);
    } catch (IOException e) {
        readEOF = true;
        return 0;
    }
  ...
  override ulong seek(long offset, SeekPos rel) {
    ...
    Anchor a;
    switch (rel) {
        case SeekPos.Set: a = Anchor.Begin; break;
        case SeekPos.Current: a = Anchor.Current; break;
        case SeekPos.End: a = Anchor.End; break;
    }
    uint result;
    try {
        result = hFile.seek(offset, a);
    } catch (IOException e) {
...
class EndianStream : FilterStream {
   ...
   */
  this(Stream source, Endian end = stdx.system.endian) {
    super(source);
    endian = end;
  }
  ...
  unittest {
    ...
    em.fixBO(x3.ptr,12);
    if (stdx.system.endian == Endian.LittleEndian) {
      assert( x3[0] == 12 );
      assert( x3[1] == 11 );
      assert( x3[2] == 10 );
      assert( x3[4] == 8 );
    ...
    em.position(0);
    em.fixBO(x3.ptr,12);
    if (stdx.system.endian == Endian.BigEndian) {
      assert( x3[0] == 12 );
      assert( x3[1] == 11 );
      assert( x3[2] == 10 );
      assert( x3[4] == 8 );
