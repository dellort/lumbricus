...
{   size_t i;
	...
	{
	    if (i >= s.length)
		throw new ArrayBoundsError("stdx.string.sformat", 0);
	    s[i] = cast(char)c;
	    ++i;
	}
	else
...
unittest
{
    debug(string) printf("stdx.string.format.unittest\n");
...
bool isNumeric(TypeInfo[] _arguments, va_list _argptr)
{
    ...
    void*[1] arglist_st;
    void*[] arglist = getArgs(_arguments, _argptr, arglist_st);
    void* arg0 = arglist[0];

    if (_arguments[0] == typeid(char[]))
        return isNumeric(*cast(char[]*)arg0);
    else if (_arguments[0] == typeid(wchar[]))
        return isNumeric(stdx.utf.toUTF8(*cast(wchar[]*)arg0));
    else if (_arguments[0] == typeid(dchar[]))
        return isNumeric(stdx.utf.toUTF8(*cast(dchar[]*)arg0));
    ...
    {
        ws.length = 1;
        ws[0] = *cast(wchar*)arg0;
        return isNumeric(stdx.utf.toUTF8(ws));
    }
    else if (_arguments[0] == typeid(dchar))
    {
        ds.length =  1;
        ds[0] = *cast(dchar*)arg0;
        return isNumeric(stdx.utf.toUTF8(ds));
    }
...
unittest
{
    ...
    assert(isNumeric(stdx.string.toString(real.nan)) == true);
    assert(isNumeric(stdx.string.toString(-real.infinity)) == true);
    assert(isNumeric(stdx.string.toString(123e+2+1234.78Li)) == true);
