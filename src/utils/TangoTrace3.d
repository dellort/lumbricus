...
class TangoTrace3Info : Exception.TraceInfo {
	int opApply(int delegate(ref Exception.FrameInfo) dg) {
		foreach (i, it; items) {
			/+char[] func, file;
			int line;
			...
			addrToSymbolDetails(it, hProcess, (char[] func, char[] file, int line, ptrdiff_t addrOffset) {
				...
				finfo.func = demangledFunc(func);
				finfo.file = file;
				finfo.line = line;
				finfo.offset = addrOffset;
				finfo.address = it;
				ret = dg(finfo);
			});
...
extern(C) void _initLGPLHostExecutableDebugInfo(char[] progName) {
