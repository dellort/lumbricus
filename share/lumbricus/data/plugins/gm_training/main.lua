...
-- targets the player has to hit
local COUNT = 5
-- time the player has (nil if unlimited)
local TIMELIMIT = time("1.5min")

local targets_left = COUNT
local timer_limit

addGlobalEventHandler("game_start", function() cospawn(initthread) end)

-- can be used to hit the target
local weapons_useful = {
    w_bazooka = 10,
}

-- helpers for worm movement
local weapons_tools = {
    w_jetpack = 25,
    w_superrope = 25,
    w_beamer = 5,
...
function initthread()
    ...
    for w, q in pairs(table_merge(weapons_useful, weapons_tools)) do
        trainee:team():addWeapon(lookupResource(w), q)
    end
    -- start target practice
    if TIMELIMIT then
        timer_limit = addTimer(TIMELIMIT, lost)
        local hud_status = TimeStatus_ctor()
        hud_status:set_showGameTime(true)
        raiseGlobalEvent("game_hud_add", "timer", hud_status)
        -- meh - must update display manually
        addWorkTimer(time("1s"), function()
            local left = timer_limit:timeLeft()
            if left then
    ...
    timer_target:setDuration(time("1min"))
...
function check()
    ...
    ws:iterate(function(weapon, count)
        if not weapons_ok then
            if weapons_useful[weapon:name()] then
                weapons_ok = true
            end
        end
    end)
...
function ending(win)
    ...
    killAll()
    addTimer(time("2s"), function() Control:endGame() end)
end
